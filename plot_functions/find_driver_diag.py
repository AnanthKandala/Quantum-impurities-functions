...
from .plot_find_runs_helper import write_title
from qim_functions import ander_in, log_couplings, extract_t_star, extract_excite
import os


def driver_check(path, tuning_var_name, outimage,s, selected_ind=[]):
    ...
    misc_string = iexcdr_deocder( inputs['iexcdr'])
    #title for the plot:
    ...
    for phase0, run_ind in runs:
        ...
        if phase in marker:
                ...
                ax.plot(level['n'], level['excite'], lw=lw, color=color[phase])
                ax.plot(level['n'][mask], level['excite'][mask], lw=0, color=color[phase], marker='X', markersize=ms)
                ax.plot(level['n'][~mask], level['excite'][~mask], lw=0, color=color[phase], marker=marker[phase], markersize=ms)
